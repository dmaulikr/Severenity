    ...
    placesClient.lookUpPhotosForPlaceID(place.placeID) { (metadata, error) in
      ...
      if let metadata = metadata {
        ...
        if !metadata.results.isEmpty {
          // NOTE: The following line is only required to work around a bug in earlier versions of
          // the Swift compiler. If you have a recent version of Xcode this cast is not needed.
          let results = metadata.results as! [GMSPlacePhotoMetadata]
          // If there were load the first one.
          self.loadPhoto(results[0])
        } else {
